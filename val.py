...
def test(args):
    ...
    testloader = DataLoader(
        test_dataset, batch_size=1, shuffle=False, num_workers=cfg.num_workers, collate_fn=test_dataset.__collate_fn__,
        pin_memory=True
    )
...
def evaluate(fuse_out_folder):
    ...
    for data_ir, data_vi, _, img_name in iter:
        ...
        # metric_result[1].update(sd_val)
        metric_result[2].update(sf_val)
        metric_result[3].update(ag_val)
        metric_result[4].update(mi_val)
        # metric_result[5].update(scd_val)
        metric_result[6].update(viff_val)
        metric_result[7].update(qabf_val)

        per_image_metrics[img_name[0]] = [en_val, sf_val, ag_val, mi_val, viff_val, qabf_val]
...
if __name__ == "__main__":
    ...
    parse.add_argument('--ckpt_path', type=str, default=f'models/model-MSRS_4.3a_epoch200.pth')
    parse.add_argument('--dataset_name', type=str, default=cfg.dataset_name)
    parse.add_argument('--out_dir', type=str, default=f'test_result/MSRS_4.3a')
